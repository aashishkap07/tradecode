...
**Shipped: C487.** The operator asked what could make the bot profitable. The
honest answer, measured below, is that **no change inside the current design has
been shown to do it**. What shipped is the fix that makes every number the bot
reports true, because paper was flattering it by about the size of the whole
result, and live mode had a gap that would have booked positions that did not exist.

- **A resting order is filled only by what the market does AFTER it arrives.**
  Paper used to judge an entry limit by the 5 minutes *before* it was placed
  and filled 92% of them (186 of 203). Now the order rests for
  `LIMIT_ORDER_TIMEOUT_SECONDS` (30 s, already in Config and never read before)
  and fills only if one of three things happens:
  - the best price on the other side reaches it;
  - a trade prints *through* it;
  - trades *at* its price use up the size that was queued ahead of it when it
    arrived, plus its own size.
  Otherwise nothing is booked (C286's unfilled path). A maker exit rests
  `C487_EXIT_REST_S` = 5 s, then crosses.
- **Live mode can no longer book a phantom position.** The live code booked a
  position the moment the order was *sent* and never read it back. The
  `pending_limits` machinery that should have done that is never filled
  (dead since it was written). A resting bid that price ran away from would have
  been managed, stopped and "closed" on paper while nothing existed on
  the exchange, or filled hours later, unmanaged. Now every order is read back:
  - filled: booked at the real average price;
  - still resting at the deadline: cancelled and read again (a fill can race the
    cancel);
  - partial: booked at the filled size, with the unused margin released;
  - unreadable: not booked, with an ERROR line to check the exchange by hand.
  Maker exits no longer leave a limit resting behind the market close, and
  market closes take the real fill price instead of the last mark.
- **Switch:** `C487_HONEST_FILLS = False` reproduces the C486 paper order exactly
  (tested on 12 cases).
- **Expect:** roughly half as many paper entries, and worse-looking paper
  results. That is the truth arriving, not a regression. No entry, exit, sizing or
  risk rule changed.
...
- `omega_c487_test.py`, 56 checks:
  - the paper order rests;
  - the fill test: book, trade-through, the queue, dedupe, and prints from
    before arrival;
  - paper settle and its timing;
  - live settle against a simulated venue: filled, resting-then-cancelled,
    partial, a fill racing the cancel, unreadable;
  - the entry, maker-exit and maker-half code executed from the shipped source,
    including partials and blended fees;
  - a sweep that every order the bot places is settled, with a positive control
    that flags all 6 in C486;
  - the switch reproduces C486 12/12;
  - negative control: C486 filled the resting entry on the spot.
- Real market, through the bot's own ccxt connection:
  - 12 resting orders on 6 coins, with queues read from the live book; 10 filled
    in that 30 s window;
  - a market close read back at its real price;
  - the full bot booted in paper and scanned for 20 minutes.
- The full battery: 25 of 25 test files pass. Both sweeps are clean.

## ⚠️ LIMITS

- The queue model credits nothing for orders cancelled ahead, so paper can only
  err toward *not* filling.
- The 30 s and 5 s waits are the existing timeout and a stated choice, not
  tuned.
- An entry blocks the scan thread for up to 30 s. A maker exit holds the close
  lock for up to 5 s.
- Live partial fills were tested against a simulated venue only. They have never
  run against Bitget.
- Tools: `omega_c487_fill_bench.py LOGDIR WORKDIR` and
  `omega_c487_edge_bench.py WORKDIR [1D|4H]`.
